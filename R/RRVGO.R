...
f <- arrow::read_feather(
    glue::glue('projects/{args[1]}/featherFiles/GOEA_{args[3]}.feather'))
...
scores <- setNames(f$p.value, f$term.id)
...
output <- file.path('projects',
                    glue::glue('{args[1]}'),
                    'featherFiles',
                    glue::glue('simMatrix{args[3]}_th{as.integer(as.double(args[5]))*100}.feather')
                    )

arrow::write_feather(simMatrix, output)

output <- file.path('projects',
                    glue::glue('{args[1]}'),
                    'featherFiles',
                    glue::glue('simReduced{args[3]}_th{as.integer(as.double(args[5]))*100}.feather')
)

arrow::write_feather(reduceSim, output)
